...
void TicTacToe(int *points, int level)
{
	...
	next_round: //if three rounds are NOT complete
	level++;
	for(i=0;i<9;i++)
	{
...
if (choice==4)
	...
	while(wrong!=0)		//For 5 tries only
	{
		...
		else
		{
			...
			PlaySound(TEXT("girl3.wav"),NULL,SND_ASYNC);
				system("cls");
				game99(0,0);
				break;
			}
...
void skeleton()
{PlaySound(TEXT("afterreaper.wav"),NULL,SND_ASYNC);
...
getch();
PlaySound(TEXT("aftergirl2.wav"),NULL,SND_ASYNC);
system("cls");
init5();
}
...
void girl1()
{PlaySound(TEXT("waker.wav"),NULL,SND_ASYNC);
...
controls();
getch();
PlaySound(TEXT("shakey.wav"),NULL,SND_ASYNC);
Waker();
getch();
system("cls");
CLockAnimation();
